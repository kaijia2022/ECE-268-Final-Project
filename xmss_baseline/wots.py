...
def read_file(file_name):
    '''
    Read the full contents of a plaintext file into memory.

    Centralises the file read so both the standalone WOTS+ digest and the
    randomized XMSS H_msg work from exactly the same message bytes.
...
def hash_file(file_name, SEED):
    ...
    return H_msg(SEED, read_file(file_name))
...
def key_gen(public_seed, seed=None, ots_address=0):
    '''
    Generate a WOTS+ public/private key pair.

    The private key contains `NUM_KEY` 32-byte atoms. The public key is
    produced by taking every private-key atom to the end of its WOTS+ chain.

    When a `seed` is supplied, the private-key atoms are derived with the
    canonical RFC 8391 pseudorandom expansion `sk[i] = PRF(seed, toByte(i, 32))`
    (the same construction the reference XMSS implementation's `expand_seed`
    uses), instead of an ad-hoc `SHA256(seed || ctr)`. This gives the PRF its
    domain-separation prefix and a full 32-byte counter.
    ...
    Args:
        ...
        ots_address: OTS address word for this WOTS+ instance. Within an XMSS
            tree this is the leaf index, so each one-time key pair uses a
            distinct address and therefore distinct chain keys/bitmasks.
    ...
    if seed is None:
        private_key = [secrets.token_bytes(KEY_SIZE) for _ in range(NUM_KEY)]
    else:
        assert isinstance(seed, bytes), "seed must be bytes"
        # Canonical RFC 8391 private-key expansion: PRF keyed by the per-OTS
        # secret seed over a 32-byte big-endian counter.
        private_key = [PRF(seed, i.to_bytes(KEY_SIZE, "big")) for i in range(NUM_KEY)]
    ...
    adrs = ADRS()
    adrs.set_type(0)
    adrs.set_ots_address(ots_address)
...
def to_digits(digest):
    '''
    Convert a 32-byte message digest into the full WOTS+ base-w digit vector.

    Splits the digest into `len_1` base-w digits and appends the `len_2`
    checksum digits. Both the standalone WOTS+ path and the randomized XMSS
    message hash funnel through here so the chaining counts always match.

    Args:
        digest: 32-byte message digest (H_msg output, possibly randomized).

    Returns:
        A list `B = M + checksum`, with length `NUM_KEY`.
    '''
    check_bytes(digest, "digest")
    M = base_w(digest, len_1, w)
...
def get_msg_digits(file_name, SEED):
    ...
    Returns:
        A list `B = M + checksum`, with length `NUM_KEY`.
    '''
    return to_digits(hash_file(file_name, SEED))


def sign_digits(B, public_seed, sk, ots_address=0):
    '''
    Produce a WOTS+ signature over a precomputed base-w digit vector.

    Each private-key atom is chained forward by the corresponding digit. This
    is the address-aware core shared by the standalone WOTS+ `sign` and by
    XMSS, which signs its randomized message digest at a specific leaf.

    Args:
        B: Base-w digit vector of length `NUM_KEY` (message + checksum).
        public_seed: 32-byte public seed used by the WOTS+ chain function.
        sk: WOTS+ private key returned by `key_gen`.
        ots_address: OTS address word (the leaf index inside an XMSS tree).
    ...
    check_bytes(public_seed, "public_seed")
    check_key(sk, "sk")
    assert len(B) == NUM_KEY, "digit vector has incorrect length"
...
def pk_from_sig_digits(B, public_seed, signature, ots_address=0):
    '''
    Recompute a WOTS+ public key from a signature and digit vector.

    Completes each chain from its message digit to the end, yielding the
    candidate public key. Shared by the standalone WOTS+ `verify` and by XMSS,
    which feeds the recovered public key into the L-tree.

    Args:
        B: Base-w digit vector of length `NUM_KEY` (message + checksum).
        public_seed: 32-byte public seed used by the WOTS+ chain function.
        signature: WOTS+ signature, a list of `NUM_KEY` 32-byte atoms.
        ots_address: OTS address word (the leaf index inside an XMSS tree).

    Returns:
        The recomputed WOTS+ public key as a list of `NUM_KEY` atoms.
    '''
    check_bytes(public_seed, "public_seed")
    check_key(signature, "signature")
    assert len(B) == NUM_KEY, "digit vector has incorrect length"
    ...
    for i in range(len(B)):
        adrs.set_chain_address(i)
        pk.append(chain(signature[i], B[i], w - 1 - B[i], public_seed, adrs))

    check_key(pk, "recomputed_pk")
    return pk
...
def sign(file_name,public_seed, sk):
    ...
    return sign_digits(B, public_seed, sk)
...
def verify(file_name,public_seed,pk,signature):
    ...
    B = get_msg_digits(file_name, public_seed)
    recomputed = pk_from_sig_digits(B, public_seed, signature)
